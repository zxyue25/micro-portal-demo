...
function callApi({ url, data = {}, options = {}, contentType = '' }) {
  if (!url) {
  ...
  if (['get', 'head'].indexOf(method) === -1) {
    ...
    if (data instanceof FormData) {
      ...
      (
        (newOptions.headers[method] || newOptions.headers)['Content-Type'] || ''
...
const restful = {
  ...
  get(url, params, options, contentType) {
    return callApi({
      url,
      options: { params, method: 'get', ...options },
      contentType,
    })
  },
...
export { setInstanceDefaultOptions, axiosInstance, restful, contentTypes }
